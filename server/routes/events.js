...
router.get("/events/article/:id", async (req, res) => {
  console.log(
    `[Events] Received request for article with ID: ${req.params.id}`
  );
  try {
    const collection = db.collection("events");
    ...
    console.error(
      `[Events] Error fetching article with ID ${req.params.id}:`,
      error
    );
...
router.patch("/events/article/:id", async (req, res) => {
  console.log(
    `[Events] Received request to update article with ID: ${req.params.id}`
  );
  try {
    const collection = db.collection("events");
    ...
    console.log(
      `[Events] Update query for article ID ${req.params.id}:`,
      update
    );
    ...
    if (result.modifiedCount === 0) {
      return res
        .status(404)
        .json({
          message: "Evento não encontrado ou nenhum campo foi modificado.",
        });
    }
    ...
    console.error(
      `[Events] Error updating article with ID ${req.params.id}:`,
      error
    );
...
router.delete("/events/article/:id", async (req, res) => {
  ...
  try {
    ...
    console.error(
      `[Events] Error deleting article with ID ${articleId}:`,
      error
    );
...
router.post("/events/article/:id/feedback", async (req, res) => {
  ...
  const articleId = req.params.id;
  if (!userId)
    return res.status(400).json({ message: "[Events] Missing userId." });
  ...
  try {
    ...
    if (!type) {
      ...
      console.log(
        `[Events] Removing feedback for article ID: ${articleId} from user ID: ${userId}`
      );
      ...
      console.log(
        `[Events] Received feedback for article ID: ${articleId} from user ID: ${userId}`
      );
      const article = await collection.findOne({
        _id: new ObjectId(articleId),
      });
    ...
    console.error(
      `[Events] Error handling feedback for event ID ${articleId}:`,
      error
    );
...
router.get("/events/article/:id/feedback", async (req, res) => {
  ...
  try {
    ...
    console.error(
      `[Events] Error fetching feedback for event ID ${articleId}:`,
      error
    );
...
router.get("/events/interacted/:userId", async (req, res) => {
  ...
  try {
    ...
    const events = await collection
      .find({ "feedback.userId": new ObjectId(userId) })
      .toArray();
